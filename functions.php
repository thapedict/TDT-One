 ...
 */
function tdt_one_filter_full_width_templates( $template ) {
    // Check to see if we are in full width
    if( ! tdt_one_site_is_full_width() ) {
        return $template;
    }

    // Don't override already set post/page templates
    if( '' !== get_page_template_slug() ) {
        return $template;
    }

    // filter for post & page
    if( ! is_single() && ! is_page() ) {
        return $template;
    }

    $tmpl_dir = trailingslashit( get_template_directory() );

    if( is_single() ) {
        $template = $tmpl_dir . 'full-width-post.php';
    }

    if( is_page() ) {
        $template = $tmpl_dir . 'full-width.php';
    }

    return $template;
}
add_filter( 'template_include', 'tdt_one_filter_full_width_templates' );
